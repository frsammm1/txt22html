...
def parse_txt_content(content):
    """ULTRA SMART TXT Parser - Handles ALL formats"""
    ...
    for line in lines:
        ...
        if not line or line.startswith('CONTENT EXPORT:') or line.startswith('ID:') or line.startswith('===') or line.startswith('---'):
            continue
        
        # Step 1: Extract link (always at the end)
        # Find LAST occurrence of http/https
...
if __name__ == '__main__':
    main(), line)
        ...
        if not link_match:
            continue  # No link found, skip line
        
        link = link_match.group(1).strip()
        
        # Step 2: Get everything BEFORE the link
        before_link = line[:link_match.start()].strip()
        
        # Remove trailing colon if exists
        if before_link.endswith(':'):
            before_link = before_link[:-1].strip()
        
        # Step 3: Extract category (in square brackets)
...
if __name__ == '__main__':
    main(), before_link)
        ...
        if not category_match:
            continue  # No category found, skip line
