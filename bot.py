...
def parse_txt_content(content):
    """Parse TXT content and categorize links - SMART VERSION"""
    ...
    for line in lines:
        ...
        if (line.startswith('CONTENT EXPORT:') or 
            line.startswith('ID:') or 
            line.startswith('===') or
            line.startswith('---')):
            continue
        
        # Check if line has format: [CATEGORY] Title: link
        if line.startswith('[') and ']:' in line:
            try:
                ...
                # Split by first colon for http/https links
                if 'http' in rest:
                    # Find the colon before http
                    parts = rest.split(':', 1)
                    if len(parts) == 2 and 'http' in parts[1]:
                        title = parts[0].strip()
                        link = parts[1].strip()
                        
                        # Remove any leading : from link
                        if link.startswith(':'):
                            link = link[1:].strip()
                        ...
                        if not link.startswith('http'):
                            ...
                            http_pos = rest.find('http')
                            if http_pos != -1:
                                title = rest[:http_pos].strip().rstrip(':')
                                link = rest[http_pos:].strip()
                        
                        # Determine file type
                        file_type = 'OTHER'
                        if '.mp4' in link.lower() or '.m3u8' in link.lower():
                            file_type = 'VIDEO'
        ...
        #                      Title: link
            ...
            # This might be a category header for next lines
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'create':
        msg = (
            ...
            "अपनी TXT file भेजें जिसमें categories और links हों!"
        )
...
async def receive_txt_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Count items
        total = sum(len(items) for items in categories.items())
        
        msg = (
            f"✅ File received!\n\n"
            f"📊 Found {len(categories)} categories with {total} items\n\n"
            f"🔐 Step 2: Set Password\n\n"
            f"HTML file के लिए एक strong password enter करें:"
        )
...
async def receive_password(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        ...
        f"📚 Step 3: Batch Name\n\n"
        f"Batch का नाम enter करें (जैसे: SSC CGL 2025):"
    )
...
async def receive_batch_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        ...
        f"Developer credit name enter करें (जैसे: @FR_SAMMM11):"
    )
...
async def process_conversion(query, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    await query.message.reply_text("⚡ Converting to HTML...")
    ...
    try:
        ...
        caption = (
            ...
            f"⚡ File ko kholein aur password enter karein!"
        )
        ...
        await query.message.reply_text(
            "🎉 Conversion Complete!\n\n"
            "Ek aur file convert karne ke liye /start karein!"
        )
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("❌ Cancelled! /start se fir shuru karein.")
    return ConversationHandler.END
...
async def error_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update and update.effective_message:
        await update.effective_message.reply_text(
            "❌ Kuch error hua! /start se fir try karein."
        )
